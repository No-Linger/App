...
export default function StatsData({
  ...
  return (
    <ScrollView>
      <View style={{ margin: 0, padding: 10, overflow: 'hidden', borderRadius: 15 }}>
        ...
        {Object.keys(dataByDate).length > 0 ? (
          <View style={{ marginTop: 10 }}>
            {Object.keys(dataByDate).map(date => (
              <View key={date} style={{ margin: -2, padding: 10, borderColor: '#4B6CFE', borderWidth: 2, borderRadius: 15 }}>
                <TouchableOpacity onPress={() => toggleDateOpen(date)}>
                  <Text style={{ fontWeight: 'bold', padding: 2, textAlign: 'left' }}>{`Fecha: ${date}`}</Text>
                </TouchableOpacity>
                {dateOpen[date] && (
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', borderColor: '#4B6CFE', borderWidth: 2, margin: 5, borderRadius: 15 }}>
            <Text style={{ padding: 20, color: '#4B6CFE', overflow: 'hidden', borderRadius: 15, fontWeight: '600', textAlign: 'center', marginTop: 10 }}>
